...
// *
// * <p>Lines 2000 ~ 2020, Flink improves error message for functions without appropriate arguments
// in
...
// * specification type at {@link
// org.apache.calcite.sql.validate.SqlValidatorImpl#validateSnapshot}.
// *
...
//
//    /**
//     * Alias generated for the target table when rewriting UPDATE to MERGE if no alias was
// specified
//     * by the user.
//     */
...
//    private final Map<IdPair<SqlSelect, Clause>, SqlValidatorScope> clauseScopes = new
// HashMap<>();
//
...
//
//    /**
//     * Stack of objects that maintain information about function calls. A stack is needed to
// handle
...
//        unknownType = typeFactory.createTypeWithNullability(typeFactory.createUnknownType(),
// true);
...
//        FunctionParamInfo funcParamInfo = requireNonNull(functionCallStack.peek(),
// "functionCall");
...
//
//    /**
//     * If <code>selectItem</code> is "*" or "TABLE.*", expands it and returns true; otherwise
// writes
//     * the unexpanded item.
//     *
...
//
//    /**
//     * Looks up completion hints for a syntactically correct select SQL that has been parsed into
// an
//     * expression tree.
//     *
...
//        } else {
//            condition =
//                    SqlStdOperatorTable.AND.createCall(SqlParserPos.ZERO, selfJoinCond,
// condition);
//        }
...
//     * self-join. If this method returns null, then no such conversion takes place. Otherwise,
// this
...
//
//    /**
//     * Returns null if there is no common type. E.g. if the rows have a different number of
// columns.
//     */
//    @Nullable
...
//        Preconditions.checkArgument(type != null, "SqlValidator.deriveTypeInternal returned
// null");
...
//        } else {
//            SqlCall testCall =
//                    resolvedConstructor.createCall(call.getParserPosition(),
// call.getOperandList());
...
//            RelDataType newInferredType = typeFactory.createTypeWithNullability(inferredType,
// true);
...
//
//            SqlNode elseOperand =
//                    requireNonNull(caseCall.getElseOperand(), () -> "elseOperand for " +
// caseCall);
...
//
//    /**
//     * Adds an expression to a select list, ensuring that its alias does not clash with any
// existing
...
//
//        final MatchRecognizeScope matchRecognizeScope = new MatchRecognizeScope(parentScope,
// call);
...
//     *
//     * <p>Likewise, {@code enclosingNode} and {@code node} are often the same. {@code
// enclosingNode}
//     * is the topmost node within the FROM clause, from which any decorations like an alias
// (<code>
...
//     *
//     * <p>The node is useful context for error messages, but you cannot assume that the node is
// the
...
//
//    /**
//     * Validates a column in a USING clause, or an inferred join key in a NATURAL join, in the
// left
...
//                (SelectScope) requireNonNull(getFromScope(select), () -> "fromScope for " +
// select);
...
//        assert fullyQualified.namespace != null : "namespace must not be null in " +
// fullyQualified;
...
//            }
//            SqlValidatorUtil.checkIdentifierListForDuplicates(columnList,
...
//        final RelOptTable relOptTable = table instanceof RelOptTable ? ((RelOptTable) table) :
// null;
...
// are
//     *     specified or if the sql dialect allows subset insert, make a subset of fields(start
// from
...
//            final SqlNode source = insert.getSource();
//            final RelDataType sourceRowType = getNamespaceOrThrow(source).getRowType();
//            final RelDataType logicalSourceRowType = getLogicalSourceRowType(sourceRowType,
// insert);
...
//                (UnpivotScope) requireNonNull(getJoinScope(unpivot), () -> "scope for " +
// unpivot);
//
...
//                                    type0, unpivot.includeNulls || unpivot.measureList.size() >
// 1);
//                    setValidatedNodeType(measure, type);
...
//                    requireNonNull(
//                            scope.getExpandedSelectList(), () -> "expandedSelectList for " +
// scope);
...
//                        // If identifier is drawn from a repeated subrecord via unnest, add name
// of
//                        // array field
...
//
//        /**
//         * Maps a column list parameter to the parent cursor parameter it references. The
// parameters
...
//     * LAST(B.price, 0)}.
//     *
//     * <p>Replacing {@code A.price} with {@code PREV(A.price, 0)} makes the implementation of
// {@link
//     * RexVisitor#visitPatternFieldRef(RexPatternFieldRef)} more unified. Otherwise, it's
// difficult
//     * to implement this method. If it returns the specified field, then the navigation such as
//     * {@code PREV(A.price, 1)} becomes impossible; if not, then comparisons such as {@code
// A.price
...
//                                ? call
//                                : SqlStdOperatorTable.LAST.createCall(SqlParserPos.ZERO,
// operands);
//                    }
//                    break;
//                default:
...
//                    this.sources =
//                            Functions.generate(
//                                    rowType.getFieldCount(), i -> ImmutableIntList.of(offset +
// i));
//                    this.trivial = true;
//            }
//        }
